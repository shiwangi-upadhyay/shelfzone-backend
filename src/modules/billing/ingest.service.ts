  ...
  return prisma.$transaction(async (tx) => {
    ...
    const taskTrace = await tx.taskTrace.create({
      data: {
        ...
        totalCost: new Prisma.Decimal((Number(totalCost) || 0).toFixed(4)),
        totalTokens,
    ...
    const masterSession = await tx.traceSession.create({
      data: {
        ...
        cost: new Prisma.Decimal((Number(input.cost) || 0).toFixed(4)),
    ...
    for (const sub of subAgents) {
      ...
      const child = await tx.traceSession.create({
        data: {
          ...
          modelUsed: sub.model,
          sessionType: input.sessionType,
          cost: new Prisma.Decimal((Number(sub.cost) || 0).toFixed(4)),
